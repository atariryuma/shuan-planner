...
export function renderWeekView(root, ctx) {
  ...
  // 提出チップ(✓提出済/未提出)はユーザー要望で撤去。提出済みフラグ(week.submittedAt)のデータ自体は
  // 残す(復元の保護・既存データ互換)。再表示したくなったらここにチップを戻すだけ。
...
function deleteCellQuick(weekStart, dayIdx, periodId, ctx) {
  ...
  // 削除後もそのコマにフォーカスを残す(行事欄などへ飛ばさない。続けて矢印移動・貼り付けができる)
  requestAnimationFrame(() => document.querySelector(`td.cell[data-day="${dayIdx}"][data-period="${CSS.escape(periodId)}"]`)?.focus());
...
export function openCellEditor(weekStart, dayIdx, periodId, ctx, opts = {}) {
  ...
  // 空きコマは「授業／予定」を選ぶ画面から始める(いきなり授業を見せて、閉じると消える混乱を防ぐ)。
  // 既存コマ・右クリック指定(asActivity/asLesson)は従来どおり直接エディタへ。
  const openedEmpty = !(store.state.weeks[weekStart]?.cells?.[editKey]?.entries?.length);
  let picking = openedEmpty && !opts.asActivity && !opts.asLesson;
  let pickSubj = null; // 専科の選択画面で「下書きの教科」(学級タップで確定)。既定は専科教科。
  ...
  // 空きコマ(picking)は選択画面から始めるので、まだ授業を充填しない。既存コマは従来どおり開いて編集。
  if (!picking) ensureLesson();
  ...
  const render = (modal) => {
    ...
    // 空きコマの選択画面: 編集モーダルと同じ「教科」「学級」パレットで選ばせる(デザインを揃える)。
    // 基本時間割の候補は淡い印で示す。専科=教科(下書き)＋学級、担任/複式=教科を選ぶと追加。
    if (picking && !cellNow.entries.length) {
      ...
      const isSenka = s.mode === 'senka' && s.senkaClasses.length;
      if (isSenka && !pickSubj) pickSubj = senkaSubjKey; // 既定の教科を下書きに
      // 教科パレット: 専科は下書き選択(selected)、担任/複式は基本時間割の候補に印(suggest)
      const subjSel = isSenka ? pickSubj : baseSubj;
      const subjPalette = `<div class="field"><label>教科${s.mode === 'fukushiki' ? '<span class="cc-hint">両学年・選ぶと追加</span>' : (!isSenka ? '<span class="cc-hint">選ぶと追加</span>' : '')}</label>
        <div class="subject-palette">${s.subjects.map(x =>
          `<button type="button" data-pick-subj="${esc(x.key)}" style="background:${esc(x.color)}" class="${x.key === subjSel ? (isSenka ? 'selected' : 'suggest') : ''}" aria-pressed="${x.key === subjSel}">${esc(x.short || x.name)}</button>`).join('')}</div></div>`;
      const scopePalette = isSenka ? `<div class="field"><label>学級<span class="cc-hint">選ぶと追加</span></label>
        <div class="scope-palette">${s.senkaClasses.map(c =>
          `<button type="button" data-pick-scope="${esc(c.id)}" class="${c.id === baseScope ? 'suggest' : ''}">${esc(c.label || '学級')}</button>`).join('')}</div></div>` : '';
      modal.querySelector('.cell-editor-body').innerHTML = `
        <div class="cell-choose">
          ${subjPalette}${scopePalette}
      ...
      // 専科: 教科は下書きを切替(学級タップで確定)。担任/複式: 教科タップで確定。
      modal.querySelectorAll('[data-pick-subj]').forEach(b => b.onclick = () => {
        if (isSenka) { pickSubj = b.dataset.pickSubj; render(modal); }
        else chooseLesson(b.dataset.pickSubj, null);
      });
      modal.querySelectorAll('[data-pick-scope]').forEach(b => b.onclick = () => chooseLesson(pickSubj || senkaSubjKey, b.dataset.pickScope));
      modal.querySelector('[data-choose-activity]')?.addEventListener('click', () => { makeActivity(); picking = false; store.commit(); render(modal); });
      // フォーカスは「選ぶと追加される」操作列へ: 専科=学級、担任/複式=教科
      modal.querySelector(isSenka ? '.scope-palette button' : '.subject-palette button')?.focus();
      return;
    }
    const ordinals = computeOrdinals(state, weekStart);
  ...
  const wireEditor = (modal) => {
    ...
    modal.querySelectorAll('[data-entry]').forEach(box => {
      ...
      // この時間で単元を終える(残りの計画コマを飛ばして次の単元へ)。状態チップ「切り上げ」のトグル。
      const euChip = box.querySelector('[data-chip-endunit]');
      if (euChip) euChip.onclick = () => { touch(); entry.endUnit = !entry.endUnit; store.commit(); render(modal); ctx.rerender(); };
    ...
    <div class="oc-titlebar">
      ...
      <details class="menu oc-menu">
        ...
        <div class="menu-items">
          ...
          <button class="btn ghost menu-item" data-oc-make-activity title="会議・出張・自習・委員会など、授業以外の予定にします（時数に数えません）">${icon('memo')}予定・活動にする</button>
...
function entryEditorHTML(state, entry, idx, period, ordinals, entriesCount = 1) {
  ...
  // 「単元の切り上げ」を出せるか(計画どおりモードで単元の途中・モジュール以外)。残りコマ数も持つ。
  // 状態チップ(時数外・中止)の隣に短い「切り上げ」チップとして出す。
  const euRemain = (lessonMode === 'plan' && details && details.unitHours > 1 && details.nth < details.unitHours && period.type !== 'module')
    ? (details.unitHours - details.nth) : 0;
  ...
  const ovField = (key, label, planVal, effVal, isOv, extra = '') => `
    <div class="ov-field${isOv ? ' is-ov' : ''}">
      <div class="ov-flabel"><b>${label}</b>
        ${isOv ? `<span class="ov-badge">変更</span><button type="button" class="ov-reset" data-ov-reset="${key}">↺ 計画に戻す</button>` : ''}
      </div>
  ...
  if (ed) {
    ...
    // 単元の切り上げは下の状態チップ(時数外・中止)の隣に「切り上げ」として出す(euRemainで判定)。
    const modeHeader = lessonMode === 'offplan'
      ...
      : ed.planless
        ...
        : `<div class="ov-head"><span class="ov-kicker">${entry.auto ? '年間指導計画' : '年間指導計画（参照）'}</span><strong>${esc(ed.unitName)}</strong>${ed.unitHours > 1 ? `<span class="ov-nth">${ed.nth}/${ed.unitHours}時</span>` : ''}<span class="ov-help">${infoHTML('計画どおりなら触らなくてOK。実際の授業に合わせて直した項目だけが「変更」として記録されます')}</span></div>`;
    ...
    <div data-entry="${idx}" class="entry-editor">
      ...
      <div class="lesson-state">
        <div class="ls-row">
          ...
          ${euRemain > 0 ? `<button type="button" class="state-chip ${entry.endUnit ? 'on' : ''}" data-chip-endunit aria-pressed="${!!entry.endUnit}" title="この単元をここで終え、残り${euRemain}コマを飛ばします（予定より早く終わったとき）">切り上げ</button>` : ''}
