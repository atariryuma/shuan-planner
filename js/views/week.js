...
export function renderWeekView(root, ctx) {
  ...
  // チップは状態が見れば分かるのでツールチップは付けない(未提出=赤・提出済=緑＋日付。クリックで切替)。
  const submitChip = submittedAt
    ? `<button class="submit-chip done" id="wk-submit">✓ 提出済 ${fmtMD(new Date(submittedAt))}</button>`
    : submitDue
      ? `<button class="submit-chip due" id="wk-submit">未提出</button>`
      : '';
