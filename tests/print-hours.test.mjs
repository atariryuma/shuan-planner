...
const { defaultState, cellKey, computeOrdinals, computeHours, scopeKey, resolveEntryPlanDetails, cellHasLock, isActivity, isEntryEdited, conformEntryToPlan, entryMatchesScope, store, mergeLessonOverride, normalizeOverride } = await import('../js/store.js');
...
test('基本時間割から復元(専科): 設定済みコマに不足学級だけ追加する(既存学級は非破壊)', () => {
  const state = defaultState();
  state.settings.mode = 'senka';
  ...
  store.state = state;
  // 月p1に 5年1組 だけ残し、5年2組 は消した状態
  state.weeks[WEEK] = { start: WEEK, cells: {
    [cellKey(0,'p1')]: { entries: [{ id:'keep', subjectKey:'rika', scope:'c1', auto:true }] },
  }, events: [], dayNotes: [], attendance: [], dayPatterns: {}, goals: '', reflection: '' };
  assert.equal(typeof store.baseRestoreLabel(WEEK, 0, 'p1'), 'string'); // 不足(5年2組)があるので導線が出る
  const ok = store.restoreCellFromBase(WEEK, 0, 'p1');
  assert.equal(ok, true);
  const scopes = store.state.weeks[WEEK].cells[cellKey(0,'p1')].entries.map(e => e.scope).sort();
  assert.deepEqual(scopes, ['c1', 'c2']);                              // 5年2組が追加された
  assert.equal(store.state.weeks[WEEK].cells[cellKey(0,'p1')].entries.find(e=>e.scope==='c1').id, 'keep'); // 既存は非破壊
  assert.equal(store.baseRestoreLabel(WEEK, 0, 'p1'), null);          // もう不足なし→導線は出ない
});
