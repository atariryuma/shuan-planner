...
test('他担当: 「本時を選ぶ」で他担当の単元を教えた分は、時数に数え単元にも実施を出す', () => {
  const state = defaultState();
  state.settings.grade = 5;
  state.plans = [{ id:'pl', subjectKey:'rika', grade:5, startOffset:0, units:[
    { id:'u1', name:'単元A', hours:2, lessons:[] },
  ...
  state.weeks = { [W]: { start:W, cells: {
    [cellKey(0,'p1')]: { entries:[{ id:'e1', subjectKey:'rika', scope:'', auto:true }] },
    [cellKey(0,'p2')]: { entries:[{ id:'e2', subjectKey:'rika', scope:'', auto:true }] },
    // 「本時を選ぶ」で他担当の単元Bを肩代わり(pin)
    [cellKey(1,'p1')]: { entries:[{ id:'e3', subjectKey:'rika', scope:'', auto:true, pin:{ unitId:'u2', nth:1 } }] },
  }, events:[], dayNotes:[], attendance:[], dayPatterns:{}, goals:'', reflection:'' } };
  store.state = state;

  // 時数集計(公式)は3コマすべて数える(肩代わり分も実授業)
  assert.equal(computeHours(store.state, W).get(scopeKey('rika', '')).done, 3);

  const f = computeProgressForecast(store.state, W).get(scopeKey('rika', ''));
  assert.equal(f.planTotal, 4);              // 自分の計画は A+C(他担当Bは除外のまま=見込みは汚さない)
  assert.equal(f.taught, 2);                  // 計画の進捗は自分の単元のみ
  assert.equal(f.units[1].status, 'shared');  // 単元B=他担当
  assert.equal(f.units[1].done, 1);           // が、肩代わりして教えた1コマは単元Bに出す

  // pinが通常の単元(自分の単元)の差し込み・再実施なら、従来どおり進度に数えない
...
test('前年度の年間行事を翌年度へ複製(年度はじめ軽量スタート)', () => {
  const state = defaultState();
  const src = '2024-04-08'; // FY2024の月曜
  state.weeks = { [src]: { start:src, cells:{}, events:['入学式','','','','',''], dayNotes:[], attendance:[], dayPatterns:{}, goals:'', reflection:'' } };
  store.state = state;
  const n = store.carryOverEvents(2024, 2025);
  assert.equal(n, 1);
  const carried = Object.entries(store.state.weeks).filter(([ws, w]) => ws !== src && (w.events || []).includes('入学式'));
  assert.equal(carried.length, 1);                       // 翌年度の同じ週番号に1件複製
  const [tws, tw] = carried[0];
  assert.ok(tws.startsWith('2025-04') || tws.startsWith('2025-03'), '翌年度の4月頃に入る: ' + tws);
  assert.equal(tw.events[0], '入学式');                  // 同じ曜日位置(月曜)を維持
});
